...
class ImportJobModelAbstract(CreateUpdateMixIn):
    ...
    ledger = models.ForeignKey('ledger.LedgerModel',
...
class StagedTransactionModelAbstract(CreateUpdateMixIn):
    uuid = models.UUIDField(default=uuid4, editable=False, primary_key=True)
    import_job = models.ForeignKey('ledger.ImportJobModel',
                                   on_delete=models.CASCADE)
    earnings_account = models.ForeignKey('ledger.AccountModel',
                                         on_delete=models.CASCADE,
                                         null=True,
                                         blank=True)
    ...
    tx = models.OneToOneField('ledger.TransactionModel',
