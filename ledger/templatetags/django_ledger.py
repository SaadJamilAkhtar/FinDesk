...
from ledger.forms.app_filters import EntityFilterForm, ActivityFilterForm
from ledger.forms.feedback import BugReportForm, RequestNewFeatureForm
from ledger.io.io_mixin import validate_activity
from ledger.models import TransactionModel, BillModel, InvoiceModel, EntityUnitModel
...
from ledger.utils import get_default_entity_session_key, get_end_date_from_session, prepare_context_by_unit
...
@register.inclusion_tag('django_ledger/tags/cash_flow_statement.html', takes_context=True)
def cash_flow_statement_table(context, io_model, from_date, to_date):
    user_model: EntityUnitModel = context['user']
    activity = context['request'].GET.get('activity')
    activity = validate_activity(activity, raise_404=True)
    entity_slug = context['view'].kwargs.get('entity_slug')
                ...
                'links': [
                    {
                        'type': 'link',
                        'title': 'Vendors',
                        'url': reverse('ledger:vendor-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                    {
                        'type': 'link',
                        'title': 'Customers',
                        'url': reverse('ledger:customer-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                    {
                        'type': 'link',
                        'title': 'Bank Accounts',
                        'url': reverse('ledger:bank-account-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                    {
                        'type': 'link',
                        'title': 'Bills',
                        'url': reverse('ledger:bill-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                    {
                        'type': 'link',
                        'title': 'Invoices',
                        'url': reverse('ledger:invoice-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                    {
                        'type': 'link',
                        'title': 'Purchase Orders',
                        'url': reverse('ledger:po-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                    {
                        'type': 'link',
                        'title': 'Inventory',
                        'url': reverse('ledger:inventory-list', kwargs={'entity_slug': ENTITY_SLUG})
                    }
                ...
                'links': [
                    {
                        'type': 'link',
                        'title': 'Entity Units',
                        'url': reverse('ledger:unit-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                    {
                        'type': 'link',
                        'title': 'My Products & Services',
                        'url': reverse('ledger:product-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                    {
                        'type': 'link',
                        'title': 'Things I Pay For',
                        'url': reverse('ledger:expense-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                    {
                        'type': 'link',
                        'title': 'Inventory Items',
                        'url': reverse('ledger:inventory-item-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                    {
                        'type': 'link',
                        'title': 'Unit of Measures',
                        'url': reverse('ledger:uom-list', kwargs={'entity_slug': ENTITY_SLUG})
                    },
                ]
            },
            {
