...
from ledger.forms.app_filters import AsOfDateFilterForm, EntityFilterForm
from ledger.forms.entity import EntityModelUpdateForm, EntityModelCreateForm
from ledger.io.data_generator import EntityDataGenerator
from ledger.models import (EntityModel, EntityUnitModel, ItemThroughModel, TransactionModel,
...
class FiscalYearEntityModelDashboardView(LoginRequiredMixIn,
    ...
    def get_context_data(self, **kwargs):
        ...
        context['pnl_chart_endpoint'] = reverse(f'ledger:{url_pointer}-json-pnl',
                                                kwargs=KWARGS)
        context['payables_chart_id'] = f'djl-entity-payables-chart-{randint(10000, 99999)}'
        context['payables_chart_endpoint'] = reverse(f'ledger:{url_pointer}-json-net-payables',
                                                     kwargs=KWARGS)
...
# CASH FLOW STATEMENT ------------
class EntityModelCashStatementView(LoginRequiredMixIn, RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        year = localdate().year
...
class FiscalYearEntityModelCashStatementView(LoginRequiredMixIn,
                                               SessionConfigurationMixIn,
                                               BaseDateNavigationUrlMixIn,
                                               EntityUnitMixIn,
                                               YearlyReportMixIn,
    ...
    context_object_name = 'entity'
    slug_url_kwarg = 'entity_slug'
    template_name = 'django_ledger/cash_flow_statement.html'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['page_title'] = _('Cash Flow Statement: ') + self.object.name
        context['header_title'] = _('Cash Flow Statement: ') + self.object.name
        unit_slug = self.kwargs.get('unit_slug')
        if unit_slug:
            context['unit_model'] = get_object_or_404(EntityUnitModel,
                                                      slug__exact=unit_slug,
...
class QuarterlyEntityModelCashStatementView(QuarterlyReportMixIn, FiscalYearEntityModelCashStatementView):
    """
    Quarter Cash Statement View.
    """


class MonthlyEntityModelCashStatementView(MonthlyReportMixIn, FiscalYearEntityModelCashStatementView):
    """
    Monthly Cash Statement View.
    """


class DateModelCashStatementView(DateReportMixIn, FiscalYearEntityModelCashStatementView):
    """
    Date Cash Statement View.
    """
...
class SetDefaultEntityView(LoginRequiredMixIn, RedirectView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            self.url = reverse('ledger:entity-dashboard',
                               kwargs={
            ...
            try:
                ...
                self.url = reverse('ledger:entity-list')
...
class SetSessionDate(LoginRequiredMixIn, RedirectView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if as_of_form.is_valid():
            ...
            self.url = reverse('ledger:entity-dashboard-date',
                               kwargs={
                                   'entity_slug': self.kwargs['entity_slug'],
                                   'year': end_date.year,
