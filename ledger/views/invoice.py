...
from ledger.forms.invoice import InvoiceModelUpdateForm, InvoiceModelCreateForm, get_invoice_item_formset
    ...
    def get(self, request, entity_slug, invoice_pk, *args, **kwargs):
        ...
        if self.action_mark_as_paid:
            ...
            messages.add_message(request,
                                 ...
                                 extra_tags='is-success')
            redirect_url = reverse('ledger:invoice-detail',
                                   kwargs={
                                       'entity_slug': self.kwargs['entity_slug'],
        ...
        if self.action_lock_ledger:
            if not ledger_model.locked:
                ...
                return HttpResponseRedirect(reverse('ledger:invoice-update',
                                                    kwargs={
                                                        'entity_slug': entity_slug,
                                                        'invoice_pk': invoice_pk
                                                    }))
