...
from ledger.forms.bill import BillModelCreateForm, BillModelUpdateForm, BillItemFormset, BillModelConfigureForm
from ledger.models import EntityModel, PurchaseOrderModel, LedgerModel
from ledger.models.bill import BillModel
from ledger.views.mixins import LoginRequiredMixIn
        ...
        if self.action_mark_as_paid:
            if not bill_model.paid:
                ...
                messages.add_message(request,
                                     ...
                                     extra_tags='is-success')
                redirect_url = reverse('ledger:bill-update',
                                       kwargs={
                                           'entity_slug': self.kwargs['entity_slug'],
                                           'bill_pk': bill_model.uuid
                                       })
        ...
        if self.action_update_items:
            ...
            if item_formset.is_valid():
                if item_formset.has_changed():
                    ...
                    return HttpResponseRedirect(reverse('ledger:bill-update',
                                                        kwargs={
                                                            'entity_slug': entity_slug,
                                                            'bill_pk': bill_pk
                                                        }))
...
class BillModelDeleteView(LoginRequiredMixIn, DeleteView):
    ...
    def get_template_names(self):
        if self.void:
            return 'ledger/bill_void.html'
        return 'ledger/bill_delete.html'
    ...
    def get_form_action_url(self):
        ...
        if self.void:
            return reverse(
                viewname='ledger:bill-void',
                kwargs=KWARGS
            )
        return reverse(
            viewname='ledger:bill-delete',
            kwargs=KWARGS
        )
        ...
        if self.void:
            ...
            if bill_model.void:
                messages.add_message(
                    ...
                    level=messages.ERROR,
                    extra_tags='is-danger')
                url_redirect = reverse('ledger:bill-detail',
                                       kwargs={
                                           'entity_slug': self.kwargs['entity_slug'],
                                           'bill_pk': bill_model.uuid
                                       })
